...
public final class RenderingUtil {
	...
	public static Pair<Artifact, Artifact> asRenderInput(ArtifactDelta delta) {
		...
		if (delta.getTxDelta() == null
			...
			ModificationType startModType = delta.getStartArtifact()
					.getModType();
			if (startModType.isDeleted()) {
				artFile1 = delta.getStartArtifact();
				artFile2 = null;
			} else if ((startModType == ModificationType.INTRODUCED || startModType == ModificationType.NEW)
					&& delta.getEndArtifact() == null) {
				artFile1 = null;
			...
			if (modType.isDeleted()) {
				artFile1 = delta.getStartArtifact();
				artFile2 = null;
			} else if ((modType == ModificationType.INTRODUCED || modType == ModificationType.NEW)
					&& delta.getStartArtifact() == null) {
				artFile1 = null;
				artFile2 = delta.getEndArtifact();
			} else {
				artFile1 = delta.getStartArtifact();
	...
	public static String getFilenameFromArtifact(FileSystemRenderer renderer,
			Artifact artifact, PresentationType presentationType)
	...
	public static IFolder ensureRenderFolderExists(
			PresentationType presentationType) throws OseeCoreException {
	...
	public static Set<Artifact> checkForTrackedChangesOn(Artifact artifact)
			throws OseeCoreException {
		Set<Artifact> artifacts = new HashSet<Artifact>();

		if (!UserManager.getUser().getBooleanSetting(
				MsWordPreferencePage.REMOVE_TRACKED_CHANGES)) {
			if (artifact != null) {
				Attribute<?> attribute = artifact
						.getSoleAttribute(CoreAttributeTypes.WORD_TEMPLATE_CONTENT);
				if (attribute != null) {
