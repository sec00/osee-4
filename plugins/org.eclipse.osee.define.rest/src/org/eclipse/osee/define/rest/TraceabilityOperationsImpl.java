...
public final class TraceabilityOperationsImpl implements TraceabilityOperations {
   ...
   private ArtifactId getCommitArtifactId(BranchId branch, String changeId) {
      return queryFactory.fromBranch(branch).and(GitChangeId, changeId).andTypeEquals(
         GitCommit).loadArtifactIdOrSentinel();
   }
   ...
   public ArtifactId baselineFiles(BranchId branch, ArtifactReadable repoArtifact, CertBaselineData baselineData, UserId account, TransactionBuilder tx, String password) {
      ...
      ArtifactId baselineCommit = getCommitArtifactId(branch, baselineData.changeId);
      if (baselineCommit.isInvalid()) {
         gitOps.fetch(repoArtifact, password);
         baselineCommit = getCommitArtifactId(branch, baselineData.changeId);
         if (baselineCommit.isInvalid()) {
   ...
   @Override
   public TransactionToken copyCertBaselineData(UserId account, BranchId destinationBranch, ArtifactReadable repoArtifact, BranchId sourceBranch) {
      ...
      for (CertBaselineData certEvent : getBaselineData(sourceBranch, repoArtifact)) {
         baselineFiles(destinationBranch, repoArtifact, certEvent, account, tx, null);
      }
      return tx.commit();
   }
   ...
   public List<CertFileData> getCertFileData(BranchId branch, ArtifactReadable repoArtifact) {
      ...
      Map<ArtifactId, ArtifactReadable> commits =
         queryFactory.fromBranch(branch).andIsOfType(GitCommit).loadArtifactMap();
