...
public class EndpointComponent implements OteUdpEndpoint {
   ...
   public EndpointComponent(){
      int port;
      try {
         String strPort = OtePropertiesCore.endpointPort.getValue(Integer.toString(PortUtil.getInstance(EthernetUtil.getServerClientAddress()).getValidPort()));
         try{
            port = Integer.parseInt(strPort);
         } catch (Throwable th){
            port = PortUtil.getInstance(EthernetUtil.getServerClientAddress()).getValidPort();
         }
         receiver = new OteUdpEndpointReceiverImpl(new InetSocketAddress(EthernetUtil.getServerClientAddress(), port));
      } catch (IOException e) {
         e.printStackTrace();
      }
   }
