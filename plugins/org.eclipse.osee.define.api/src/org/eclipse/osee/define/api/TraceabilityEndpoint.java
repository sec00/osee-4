...
public interface TraceabilityEndpoint {
   ...
   @POST
   @Path("/cert/{destinationBranch}/repo/{repository-name}")
   @Consumes(MediaType.APPLICATION_JSON)
   @Produces(MediaType.APPLICATION_JSON)
   TransactionToken copyCertBaselineData(@HeaderParam("osee.account.id") UserId account, @PathParam("destinationBranch") BranchId destinationBranch, @PathParam("repository-name") String repositoryName, BranchId sourceBranch);
