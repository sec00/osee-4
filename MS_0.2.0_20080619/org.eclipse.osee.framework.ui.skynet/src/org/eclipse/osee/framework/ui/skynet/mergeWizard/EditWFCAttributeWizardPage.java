...
public class EditWFCAttributeWizardPage extends WizardPage {
	...
	private static final String SDIFF_TOOLTIP = "Show the differences between the current Source"
			+ " artifact and the artifact at the time the Source Branch was created";
	private static final String DDIFF_TEXT = "Show Destination Diff";
	private static final String DDIFF_TOOLTIP = "Show the differences between the current Destination"
			+ " artifact and the artifact at the time the Source Branch was created";
	private static final String SDDIFF_TEXT = "Show Source Destination Diff";
	private static final String SDDIFF_TOOLTIP = "Show the differences between the current Destination"
			+ " artifact and the current Source artifact";
			...
			try {
				if (event.widget == editButton) {
					RendererManager.getInstance().editInJob(
							conflict.getArtifact(), "PREVIEW_ARTIFACT");
				...
				OSEELog
						.logException(EditWFCAttributeWizardPage.class, ex,
								true);
			}
	...
	public void createControl(Composite parent) {
		...
		try {
			new Label(composite, SWT.NONE)
					.setText(ConflictResolutionWizard.ART_TEXT);
			new Label(composite, SWT.NONE)
					.setText(ConflictResolutionWizard.INDENT
							+ conflict.getArtifactName());
			new Label(composite, SWT.NONE)
					.setText(ConflictResolutionWizard.TYPE_TEXT);
			new Label(composite, SWT.NONE)
					.setText(ConflictResolutionWizard.INDENT + changeType);
		} catch (Exception ex) {
		...
		sourceDestDiffButton = createButton(SDDIFF_TEXT, SDDIFF_TOOLTIP,
				buttonComp);
